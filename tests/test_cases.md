...
### TC-ADMIN-001:Admin is unauthorized
**Priority:** P2
**Pre-conditions:** 
- App runs properly
...
### TC-ADMIN-002: One review per purchaser; edit & delete (FR-U01)
**Priority:** P2
**Pre-conditions:** 
...
### TC-ADMIN-003: Moderation flow — report → admin queue → action (FR-U02/FR-M04)
**Priority:** P2
**Pre-conditions:** 
...
### (Admin/Orders) TC-ADMIN-004: Order lifecycle transitions (Pending → Paid → Fulfilled → Delivered)
**Priority:** P1
**Pre-conditions:** 
...
### (Admin/Refunds) TC-ADMIN-005: Full and partial refunds (FR-R02)
**Priority:** P2
**Pre-conditions:** 
...
### (Admin/Import) TC-ADMIN-006: CSV import sanity (orders CSV import edge cases)
**Priority:** P3
**Pre-conditions:** 
...
Steps:
- On the Cart or Checkout page, locate the coupon input.
- Enter a valid coupon code and click Apply.
- Observe the discount line and new totals.
- Verify the discount amount matches the coupon definition.
Expected Result:
...
- Click Apply.
- Observe the UI message and ensure totals have not changed.
Expected Result:
...
Steps:
- Apply the non-combinable coupon and note totals.
- Attempt to apply the second coupon.
- Observe whether the second coupon is rejected and the message shown.
- Adjust cart total to below min-basket and attempt to apply coupon that requires min-basket.
Expected Result:
...
Steps:
- Click "Proceed to Checkout".
- Fill Full Name, Email, Address, City, Country, Postal Code with valid data.
- Click Next.
- Observe navigation to Review step.
Expected Result:
...
Steps:
- Leave Email field blank or type invalid email ("notanemail").
- Click Next.
- Observe validation messages and whether navigation occurs.
Expected Result:
...
Pre-conditions: Checkout Shipping step
Steps:
- Leave Full Name empty and fill other fields.
- Click Next.
- Observe the focused field and error shown.
Expected Result:
...
Steps:
- Click Back to return to Shipping step.
- Check each field contains the previously entered value.
- Edit a field and navigate forward again.
Expected Result:
...
Steps:
- On Review step, list items and line subtotals.
- Verify displayed subtotal equals sum of line items.
- Verify shipping fee present and tax = 8% of subtotal is shown.
- Verify final total calculation (subtotal + shipping + tax - discounts).
Expected Result:
...
Steps:
- On Review or Payment step, click Pay Now.
- Wait for Paystack modal/iframe to appear.
- Confirm the amount and currency shown match the app's totals.
Expected Result:
...
Steps:
- Configure or simulate an unsupported currency in the app (or use test env where currency is unsupported).
- Click Pay Now.
- Observe any preflight error message preventing transaction start.
Expected Result:
...
Steps:
- Open Paystack modal as in TC-RS-020.
- Close the modal or cancel within the modal.
- Return to the app and check order state (if order record was created).
Expected Result:
...
Steps:
- Complete the payment process in test mode (simulate success).
- Navigate to Orders/Order History page.
- Locate the newest order and open its details.
- Verify status = Paid and gatewayRef is visible.
Expected Result:
...
Steps:
- Go to Orders page and click Export CSV.
- Download the file and open in a spreadsheet application (Excel/Sheets).
- Verify columns, header row, and decimal formatting (dot as decimal separator).
Expected Result:
...
Steps:
- On Order details, click Request Return.
- Fill return reason and submit.
- Verify the return request is accepted and an audit entry recorded.
Expected Result:
...
Steps:
- As a purchaser, navigate to book page and submit a review.
- Verify the review appears.
- As a non-purchaser, attempt to submit a review and note the response.
Expected Result:
...
Steps:
- Enter a review containing a link with scheme `javascript:`.
- Submit the review.
- Observe whether input is sanitized and whether the link is blocked.
Expected Result:
...
Steps:
- On a review, click Report/Flag.
- As admin, open the moderation queue and confirm the flagged review appears.
Expected Result:
...
Steps:
- Log in as non-admin and navigate to /admin.
- Verify access blocked or unauthorized message.
- Log in as admin, go to admin Catalog, create a new book with minimal data, save it.
- Verify book appears in catalog and details page.
Expected Result:
...
Steps:
- Open notifications panel and note unread badge count.
- Click Mark All Read.
- Observe badge count and notification states.
Expected Result:
...
Steps:
- Use the Tab key to navigate interactive elements on the page.
- Ensure focus order is logical and visible.
- Inspect form elements to confirm label association.
- Run a Lighthouse or aXe quick scan (if available) and record top-level issues.
Expected Result:
...
Steps:
- Open the Catalog page in a desktop browser.
- Use browser devtools to measure LCP and TTI or note approximate load time.
- Verify images are lazy-loaded (inspect image attributes).
Expected Result:
...
- Run a simple scenario: open Catalog → add a book → open Cart → proceed to Checkout.
- Note any visible errors or deviations.
Expected Result:
